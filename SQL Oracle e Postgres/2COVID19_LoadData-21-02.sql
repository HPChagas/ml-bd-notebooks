...
Não é necessário descompactar os arquivos de ccada hospital.
...
-- COMENTAR os paths dos hopitais que não devem ser carregados.
    ZipPath:=     '7z';  -- NÃO COMENTAR --
	FilePathHSL:= '/tmp/dbMbaUsp/HSL_Junho2021.zip';
	--FilePathHFL:= '/home/icarus/MBA_AI_e_ML/02CursoAdministracaodeDadosComplexos/GrupoFleury_Junho2021.zip';
	--FilePathHEI:= '/home/icarus/MBA_AI_e_ML/02CursoAdministracaodeDadosComplexos/EINSTEINAgosto.zip';
	--FilePathHCSP:='/home/icarus/MBA_AI_e_ML/02CursoAdministracaodeDadosComplexos/BDCovid/HospitaldasClinicas_Janeiro2021.zip';
	FilePathBPSP:='/tmp/dbMbaUsp/BPSP.zip';
    Cria:='2';  -- Inclui 'T' se deve criar Schema TODOS, Inclui '2' se deve criar Schema de 2 hospitais
	Hosp2:='HSL BPSP';
...
-- Carrega dados Hospital Fleury: HFL  --------------------------------------------------------
    IF FilePathHFL IS NOT NULL THEN
        ...
        CREATE TABLE Pacientes AS TABLE Public.Pacientes WITH NO DATA;
        CREATE TABLE ExamLabs  AS TABLE Public.ExamLabs  WITH NO DATA;
        PERFORM LoadZIPFile(ZipPath, FilePathHFL, 'GrupoFleury_Pacientes_3.csv'::TEXT, 'Pacientes'::TEXT);
        PERFORM LoadZIPFile(ZipPath, FilePathHFL, 'GrupoFleury_Exames_3.csv'::TEXT,    'ExamLabs'::TEXT);
